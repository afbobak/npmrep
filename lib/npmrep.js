...
npmrep.getMatchingPkg = function (range, pkgInfo) {
	if (range === undefined) {
		...
		throw new TypeError(msg + "Expected range, got: " + range);
	}
...
npmrep.getSrcPkg = function (pkgName, range) {
	...
	this.getSrcInfo(pkgName).when(function (error, pkgInfo) {
		...
		try {
			...
			future.deliver({
				error: "failed-srcpkg",
				reason: "Failed to get source information for package " + pkgName + "@" + range,
				details: e
			});
		}
	});
		...
		if (pending[pkgId]) {
			...
			self.getSrcPkg(pkgName, range).when(function (error, pkg) {
				...
				function done() {
					...
					for (i = 0; i < l; i++) {
						if (arguments[0][0]) {
							console.error(" -> failed ".red + pkgName + "@" + pkg.version, arguments[0][0]);
							return future.deliver(arguments[0][0]);
						}
					}
